...
var grandTileCache = new tileCache({
  tileTypes: tiles,
  defaultTile: '256.png'
});
...
grandTileCache.on('cache_hit', function(data) {
  ...
  Leftronic.pushNumber({
    streamName: 'totalTiles_' + config.name,
  ...
  Leftronic.pushNumber({
    streamName: 'totalTransfer_' + config.name,
    number: stats.transfer / 1024,
    suffix: 'Kb',
  }, function(err, result) {
    if (err) console.log('left err:' + err);
    //console.log(result);
...
getTile = function(req, res) {
  ...
  // estos datos los pego al req para poder accederlos desde el tileCache
  req.requestData = {
    host: req.headers["host"],
  ...
  res.on('finish', function() {
    ...
    grandTileCache.emit('tile_served', {
      type: 'tile_served',
      tile: tile
    });
  });
...
app.httpServer.listen(port, function() {
  ...
  setInterval(intervalUpdater, 1000);
});

var intervalUpdater = function() {
  //aca todo lo que quiero enviar cada n secs
  //process.uptime()
  //process.memoryUsage()
  //{ rss: 4935680,
  // heapTotal: 1826816,
  ...
  Leftronic.pushNumber({
    streamName: 'tilesPerSecond_' + config.name,
    number: stats.tiles,
    // suffix: 'tiles/second',
  }, function(err, result) {
    if (err) console.log('left err:' + err);
  ...
  stats.tiles = 0;
  Leftronic.pushNumber({
    streamName: 'processRss_' + config.name,
    number: mem.rss / 1024 / 1024,
    // suffix: 'Kb',
  }, function(err, result) {
    if (err) console.log('left err:' + err);
    //console.log(result);
  ...
  Leftronic.pushNumber({
    streamName: 'errors_' + config.name,
    number: stats.errors,
    // suffix: 'Kb',
  }, function(err, result) {
    if (err) console.log('left err:' + err);
